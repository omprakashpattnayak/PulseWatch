...
export function Hero() {
  ...
  const { data, error, mutate } = useSWR<{ events: DemoEvent[]; source: string }>('/api/earthquakes', fetchUSGS, { refreshInterval: 300000, revalidateOnFocus: false, keepPreviousData: true })
  ...
  const events = [...liveEvents, ...wildfireEvents, ...gdacsEvents]
  const anyFeedLoading = !hasMounted || !data || !wildfireData || !gdacsData
  const anyFeedOffline = Boolean(error || wildfireError || gdacsError)
      ...
      <section className={cn('hero', exploring && 'is-exploring')} id="map" aria-labelledby="hero-title">
        ...
        <div className="page-width hero-inner">
          <div className="hero-topline">
            ...
            <div className="prototype-label" suppressHydrationWarning><span className={cn('status-dot', hasMounted && error && 'status-dot-error')} /> {anyFeedLoading ? 'CONNECTING TO LIVE SOURCES' : anyFeedOffline ? 'FEED OFFLINE' : 'LIVE SOURCES CONNECTED · USGS + NASA FIRMS + GDACS'}</div>
          </div>
          ...
          {activeSelected && (
            <aside className={cn('map-event-card', `event-card-${activeSelected.type}`)} aria-label={`Selected ${activeSelected?.type ?? 'hazard'} event`} aria-live="polite">
              <div className="event-card-top"><span><SelectedIcon size={13} aria-hidden="true" /> {eventCategories[activeSelected.type].label === 'Floods & climate' ? 'CLIMATE ALERT' : activeSelected.type.toUpperCase()}</span><span className="event-sample">LIVE EVENT</span></div>
