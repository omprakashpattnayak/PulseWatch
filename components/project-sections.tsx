...
export function ProblemSection() {
  return (
    <section id="problem" className="project-section problem-section page-width" aria-labelledby="problem-heading">
      ...
      <div className="research-stats" aria-label="Impact statistics awaiting research verification">{[{ label: 'People affected by disasters annually', source: 'UNDRR / CRED' }, { label: 'Annual global economic losses', source: 'UNDRR / WMO' }, { label: 'Response time lost switching tools', source: 'RESPONDER INTERVIEWS' }].map((stat) => <div key={stat.label}><span className="stat-pending">—<small>RESEARCH PENDING</small></span><p>{stat.label}</p><span className="verification-note">[VERIFY WITH SOURCE]</span><small className="research-source">PLANNED SOURCE: {stat.source}</small></div>)}</div>
    </section>
  )
}
